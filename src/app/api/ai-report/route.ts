...

{
  ...
  "categorySummary": מערך של קבוצות הוצאה. כל קבוצה מכילה:
    - "groupName": שם הקבוצה בעברית (בדיוק כפי שהוא מופיע בנתונים)
...
כללים חשובים לתוכן:
- כתוב עברית ישראלית טבעית, אנושית ואמפתית — כמו יועץ שמכיר את המשפחה, לא כמו תרגום מאנגלית
- אסור להשתמש במילים: "קנוניות", "לתיקום", "בר קיימא", "מיטוב", "אופטימיזציה", "סינרגיה", "רצף", "מניפה", "מדרגי"
- השתמש במילים כמו: "משמעת עצמית", "עקביות", "שינוי מגמה", "חסכון", "הוצאה", "תקציב", "מעקב", "שיפור"
...
- השקעות (is_investment=true): אל תמנה אותן כהוצאות תפעוליות, אל תציג אותן בwarnings בגלל חריגה מתקציב
- אם יש קטגוריות השקעה, שבח את המשפחה על כך ב-highlights בנוסח חם וישיר (לדוגמה: "העברתם X לחסכונות — כל שקל שנשמר עכשיו עובד בשבילכם")
- ב-tldr: אם יש השקעות, הזכר את התזרים התפעולי (ללא השקעות) ואת סכום ההשקעות בנפרד. לדוגמה: "התזרים התפעולי החודש הוא ₪X, ובנוסף הפניתם ₪Y להשקעות."
- החזר JSON בלבד, ללא markdown, ללא הסברים
- אסור בהחלט לכלול מילים באנגלית בשדות הטקסט (tldr, title, description, groupName, itemName, category, issue, actionItem) — עברית בלבד, ללא יוצא מן הכלל`
